      ...
      <AnimatePresence mode="wait">
        {analyzeMutation.data ? (
          ...
          <motion.div 
            ...
            exit={{ opacity: 0 }}
            className="flex-1 flex flex-col items-center justify-center p-8 bg-white/50 rounded-3xl border shadow-inner"
          >
            <div className="relative w-48 h-48 mb-8 rounded-2xl overflow-hidden border-4 border-primary/20 bg-black/5 shadow-2xl">
              {image && <img src={image} alt="Scanning" className="w-full h-full object-cover opacity-50" />}
              <div className="scan-line" />
            </div>
            <Loader2 className="w-8 h-8 text-primary animate-spin mb-4" />
            <p className="text-xl font-bold text-foreground animate-pulse">{scanProgress}</p>
          </motion.div>
        ) : (
          <motion.div 
            key="upload"
          ...
          >
            {!image ? (
              <div 
                {...getRootProps()} 
                className={`flex-1 min-h-[250px] border-3 border-dashed rounded-3xl flex flex-col items-center justify-center p-8 cursor-pointer transition-all duration-300 ${
              ...
              >
                ...
                <p className="text-lg font-bold text-center mb-1">Tap to Upload Image</p>
                <p className="text-sm text-muted-foreground text-center">or drag and drop here</p>
              </div>
            ) : (
              <div className="relative rounded-3xl overflow-hidden shadow-xl border bg-black">
                <button 
            ...
            <div className="mt-8 mb-6">
              <h3 className="font-bold text-lg mb-3">Checking for:</h3>
            ...
            <div className="mt-auto pt-4">
              <button
                ...
                onClick={handleScan}
                className="w-full py-4 rounded-2xl font-bold text-lg bg-gradient-to-r from-primary to-blue-500 text-white shadow-xl shadow-primary/25 disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-primary/40 hover:-translate-y-0.5 active:translate-y-0 transition-all flex items-center justify-center gap-2"
              >
